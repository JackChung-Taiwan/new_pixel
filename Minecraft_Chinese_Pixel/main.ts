...
namespace chinesePixel {
    ...
    function drawGlyph(character: string, origin: Position, direction: CompassDirection, block: number, baseX: number, baseY: number, scale: number): void {
        const rows = limitedStrokeRows(decodeGlyph(pixelFontData.glyphFor(character)))
        for (let sourceY = 0; sourceY < SOURCE_GLYPH_SIZE; sourceY++) {
            ...
            while (sourceX < SOURCE_GLYPH_SIZE) {
                ...
                const x0 = baseX + correctedStart * STROKE_SCALE * scale
                const x1 = baseX + (correctedEnd + 1) * STROKE_SCALE * scale - 1
                const y0 = baseY + (SOURCE_GLYPH_SIZE - 1 - sourceY) * STROKE_SCALE * scale
                const y1 = y0 + STROKE_SCALE * scale - 1
    ...
    function fillBlockRect(origin: Position, direction: CompassDirection, block: number, x0: number, y0: number, x1: number, y1: number): void {
        ...
        if (direction === CompassDirection.North) {
            fromPosition = world(originX - x0, originY + y0, originZ)
            toPosition = world(originX - x1, originY + y1, originZ)
        } else if (direction === CompassDirection.East) {
            fromPosition = world(originX, originY + y0, originZ - x0)
            toPosition = world(originX, originY + y1, originZ - x1)
    ...
    function limitedStrokeRows(rows: number[]): number[] {
    ...
    function thinStep(pixels: number[][], step: number): void {
        ...
        for (let y = 1; y < SOURCE_GLYPH_SIZE - 1; y++) {
            for (let x = 1; x < SOURCE_GLYPH_SIZE - 1; x++) {
                ...
                const north = getMatrixPixel(pixels, x, y - 1) ? 1 : 0
                const east = getMatrixPixel(pixels, x + 1, y) ? 1 : 0
                const south = getMatrixPixel(pixels, x, y + 1) ? 1 : 0
                const west = getMatrixPixel(pixels, x - 1, y) ? 1 : 0
                if (step === 0) {
    ...
    function transitionCount(pixels: number[][], x: number, y: number): number {
        ...
        for (let i = 0; i < ring.length; i++) {
            ...
            if (ring[i] === 0 && ring[next] === 1) transitions++
        }
        return transitions
    }
    ...
    function rowsToMatrix(rows: number[]): number[][] {
        ...
        for (let y = 0; y < SOURCE_GLYPH_SIZE; y++) {
            const row: number[] = []
            for (let x = 0; x < SOURCE_GLYPH_SIZE; x++) row.push(sourcePixelIsSet(rows[y], x) ? 1 : 0)
            matrix.push(row)
        }
        return matrix
    }
